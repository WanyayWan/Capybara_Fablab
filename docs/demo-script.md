...
| 1:30 | **Laser cutter safety.** "Even at the 3D printer, it knows the lab's safety rules." Hold: *"Can I cut PVC on the laser cutter?"* | "According to the laser cutter guide, no, you cannot cut PVC on the laser cutter. They release toxic chlorine gas that harms people and damages the machine. ..." Say: "Banned materials are always checked, on every unit, so it can't guess on safety." |
...
| PVC | Not heard or not caught | Hold and ask *"Is vinyl okay to laser cut?"* (eval Q26 wording; the banned list applies on every unit), or from the laptop: `curl.exe -s -X POST http://127.0.0.1:8000/api/ask -H "Content-Type: application/json" -d '{\"device_id\":\"fabai-01\",\"question\":\"Can I cut PVC on the laser cutter?\",\"speak\":true}'`. If the backend is down, say the answer and move on. |
