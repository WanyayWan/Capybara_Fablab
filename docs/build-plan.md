...
### Phase 5 eval decisions (2026-09-24)

First eval run: 22/23. Q20 ("How much does SLS printing cost per part?") scored 0.663 and the LLM
answered "I don't have information about the cost..." plus an SLS fact instead of `NO_ANSWER`.
Q10 was credited to "the 3D printer guide" (its only source was the laser cutter guide), and
non-procedure answers ended with "Say next when you're ready." because the prompt always asked
for it.

1. **Refusal.** The rule is now "If CONTEXT does not answer this exact question, even if related
   facts exist, reply with exactly NO_ANSWER and nothing else." The pipeline also treats a reply
   that opens with "I don't have information", "I don't have that" or "I don't know" (after any
   "According to ...,", curly apostrophes straightened) as `NO_ANSWER`: refused, logged, normal
   refusal text (`core/answer_text.is_refusal_reply`). Tests P7, AT1, AT2, PL19.
2. **Source naming is deterministic.** The prompt says "Do not mention sources or guide names."
   For QUESTION answers the pipeline strips any leading "According to ...," the model wrote and
   prefixes "According to <top chunk spoken_source>, " (first word lower-cased only if it is a
   plain capitalised word, so "I", "I'm", "SUTD" keep their case). No prefix for refusals, help,
   emergency or NEXT (NEXT via the LLM gets the model's prefix stripped; step-mode NEXT is
   verbatim, 9 step-mode review 1). Tests P10, AT3, AT4, AT6, PL20.
3. **"Say next" is added by the pipeline.** Removed from the prompt. `shape_reply` strips it
   (including a "Next," lead-in, a quoted "next", and a bare trailing "Next" after the last
   sentence, which gemma adds when staff have been called) and appends
   "Say next when you're ready." only when this answer set the step pointer. Tests P6, AT5, AT7,
   AT8, PL21, PL5b, PL7, API8.
4. **Eval runs** use a backend started with `SESSION_TIMEOUT_S=0` (each question is fresh; the
   same device would otherwise merge follow-ups) and `TELEGRAM_BOT_TOKEN` / `TELEGRAM_CHAT_ID` set
