...
"<section class='section'><h2>DEVICE</h2><div class='row'><span>Status</span><span id='device'>Loading</span></div><div class='row'><span>Device</span><span id='devid'>-</span></div><div class='row'><span>Firmware</span><span>0.1.0</span></div><div class='row'><span>Uptime</span><span id='uptime'>-</span></div></section>"
...
static esp_err_t status_handler(httpd_req_t *req)
{
    ...
    snprintf(json, sizeof(json), "{\"device_id\":\"" DEVICE_ID "\",\"uptime\":%lu,\"button\":%s,\"wifi\":{\"connected\":%s,\"setup\":%s,\"ssid\":\"%s\",\"ip\":\"%s\",\"rssi\":%d},\"backend\":{\"online\":%s,\"url\":\"%s\"}}",
...
static esp_err_t action_handler(httpd_req_t *req)
{
    ...
    if (strcmp(path, "/api/led/on") == 0) led_manager_set("white");
    else if (strcmp(path, "/api/led/off") == 0) led_manager_set("off");
